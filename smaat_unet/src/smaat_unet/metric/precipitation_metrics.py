...
class PrecipitationMetrics(Metric):
    ...
    def compute(self):
        ...
        csi = (
            self.total_tp / (self.total_tp + self.total_fn + self.total_fp)
            if (self.total_tp + self.total_fn + self.total_fp) > 0
            else torch.tensor(float("nan"))
        )

        # False Alarm Ratio (FAR)
        far = (
            self.total_fp / (self.total_tp + self.total_fp)
            if (self.total_tp + self.total_fp) > 0
            else torch.tensor(float("nan"))
        )

        # Heidke Skill Score (HSS)
        denom = (self.total_tp + self.total_fn) * (self.total_fn + self.total_tn) + (self.total_tp + self.total_fp) * (
            self.total_fp + self.total_tn
        )
        hss = (
            ((self.total_tp * self.total_tn) - (self.total_fn * self.total_fp)) / denom
            if denom > 0
            else torch.tensor(float("nan"))
        )
