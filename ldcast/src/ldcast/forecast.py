...
from ldcast.models.autoenc import encoder, autoenc
from ldcast.models.genforecast import analysis, unet
from ldcast.models.diffusion import diffusion, plms
...
class Forecast:
    ...
    def _init_model(self):
        ...
        checkpoint = torch.load(self.autoenc_weights_fn)
        ...
        # create LDM
        ldm = diffusion.LatentDiffusion(denoiser, autoencoder_obs, context_encoder=analysis_net, parameterization="eps")

        checkpoint = torch.load(self.ldm_weights_fn)
        if "state_dict" in checkpoint:
    ...
    def input_timesteps(self, x):
        ...
        timesteps = torch.arange(x.shape[2], dtype=x.dtype, device=self.ldm.device) * 300
        return (timesteps.unsqueeze(0).expand(batch_size, -1) - 900) / 300
...
class ForecastDistributed:
    ...
    def __call__(self, R_past, ensemble_members=1, num_diffusion_iters=50):
        ...
        # build output array
        pred_shape = (R_past.shape[0], self.future_timesteps) + R_past.shape[2:] + (ensemble_members,)
