...
def plot_csi(
    ...
    for i, models in enumerate(model_sets):
        for j, T in enumerate(thresholds):
            ax = fig.add_subplot(gs[i, j])
            models_T = [m.format(T=T) for m in models]
            plot_func = {"leadtime": plots.plot_csi_leadtime, "threshold": plots.plot_csi_threshold}[plot_by]
            plot_func(
                ax=ax,
                scales=(scale,),
                models=models_T,
...
def plot_sample_cases(
    models=("mch-iters=50-res=256", "mch-iters=50-res=256", "dwd-iters=50-res=256", "dwd-iters=50-res=256"),
    ...
    gs_outer = gridspec.GridSpec(N_models, 2, wspace=0.02, hspace=0.05, width_ratios=(0.98, 0.02))

    for k, (x, y, y_pred) in enumerate(data):
        gs = gridspec.GridSpecFromSubplotSpec(2, N_timesteps, subplot_spec=gs_outer[k, 0], wspace=0.02, hspace=0.02)
        for j, t in enumerate(past_timesteps):
            ax = fig.add_subplot(gs[0, j])
            plots.plot_precip_image(ax, x[0, t, :, :], transform_R=False)
        ...
        for j, t in enumerate(future_timesteps, start=len(past_timesteps)):
            ...
            im = plots.plot_precip_image(ax, y_pred[0, t, :, :, ensemble_member], transform_R=False)
...
def plot_ensemble(
    ...
    gs_outer = gridspec.GridSpec(N_cases, 2, wspace=0.02, hspace=0.05, width_ratios=(0.98, 0.02))

    t = future_timestep
    for k, (model_list, sample) in enumerate(zip(models, samples)):
        gs = gridspec.GridSpecFromSubplotSpec(
            2 * len(model_list), N_members + 1, subplot_spec=gs_outer[k, 0], wspace=0.02, hspace=0.02
        )
