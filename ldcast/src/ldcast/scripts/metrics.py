...
from ldcast.analysis import fss, rank, crps, histogram
...
def crps_all(models=all_models, prob_match=True, log=False):
    ...
    for model in models:
        ...
        crps.save_crps_for_dataset(ensembles_dir, crps_fn, log=log, preproc_fc=pm)
...
def ranks_all(models=all_models, prob_match=True):
    ...
    for model in models:
        ...
        rank.save_ranks_for_dataset(ensembles_dir, ranks_fn, preproc_fc=pm)


def fractions_all(models=all_models, thresholds=(0.1, 0.3, 1.0, 3.0, 10.0, 30.0), prob_match=True):
    ...
    for model in models:
        ...
        for T in thresholds:
            ...
            fss.save_fractions_for_dataset(ensembles_dir, fss_fn, T, preproc_fc=pm)
...
def rmse_ensemble_mean(model="mch-iters=50-res=256", log=False, prob_match=True):
    ...
    with concurrent.futures.ProcessPoolExecutor(N_threads) as executor:
        tasks = [executor.submit(rmse_ensemble_mean_batch, fn, log, pm) for fn in files]
