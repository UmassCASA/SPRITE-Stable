...
from ldcast.features import split
from ldcast.features import patches, transform

from sprite_core.config import Config

from ldcast.features.casa_data import CasaDatasetWithTimeList
from ldcast.features.casa_data import CasaDataModuleWithTimeList
...
def setup_data(
    ...
    use_nwp=False,
    obs_vars=("RZC",),
    nwp_vars=("cape", "cin", "rate-cp", "rate-tp", "t2m", "tclw", "tcwv", "u", "v"),
    nwp_lags=(0, 12),
    target_var="RZC",
    batch_size=64,
    past_timesteps=4,
    ...
    for var, raw_var in zip(predictors_obs, obs_vars):
        variables[var] = {"sources": [raw_var], "timesteps": np.arange(-past_timesteps + 1, 1)}
    nwp_t1 = int(np.ceil(future_timesteps * timestep_secs / nwp_timestep_secs)) + 2
    nwp_range = np.arange(nwp_t1)
    variables["nwp"] = {"sources": nwp_vars, "timesteps": nwp_range, "timestep_secs": nwp_timestep_secs}

    # determine which raw variables are needed, then load them
    raw_vars = set.union(*(set(variables[v]["sources"]) for v in predictors_obs + [target]))
    if use_nwp:
        for raw_var_base in variables["nwp"]["sources"]:
            raw_vars.update(f"{raw_var_base}-{lag}" for lag in nwp_lags)
    raw = {var: patches.load_all_patches(os.path.join(file_dir, f"../data/{var}/"), var) for var in raw_vars}
    ...
    transform_rain = lambda: transform.default_rainrate_transform(raw["train"]["RZC"]["scale"])
    transform_cape = lambda: transform.normalize_threshold(
        log=True, threshold=1.0, fill_value=1.0, mean=1.530, std=0.859
    )
    transform_rate_tp = lambda: transform.normalize_threshold(
        log=True, threshold=1e-5, fill_value=1e-5, mean=-3.831, std=0.650
    )
    ...
    transforms = {
        ...
        "tclw": transform.normalize_threshold(log=True, threshold=0.001, fill_value=0.001, mean=-1.486, std=0.638),
...
def setup_data_casa(for_ae=False, batch_size=8):
    print(f"data_dir: {Config.DATA_DIR}")

    datamodule = CasaDataModuleWithTimeList(
        batch_size=batch_size,
        val_batch_size=8,
        num_workers=1,
        num_input_frames=4,
        num_target_frames=20,
        ensure_2d=False,
        data_dir=Config.DATA_DIR,
        persistent_workers=False,
        dataset=CasaDatasetWithTimeList,
        for_ae=for_ae,
        dt=1,
    )
    datamodule.setup()
