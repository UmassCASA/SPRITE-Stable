...
from ldcast.scripts.train_nowcaster import setup_data_casa

file_dir = os.path.dirname(os.path.abspath(__file__))


def setup_data(var="RZC", batch_size=64, sampler_file=None, num_timesteps=8, chunks_file="../data/split_chunks.pkl.gz"):
    variables = {
        var: {
            "sources": [var],
    ...
    variables[var]["transform"] = transform.default_rainrate_transform(raw["train"][var]["scale"])
...
def setup_model(model_dir=None):
    ...
    (autoencoder, trainer) = training.setup_autoenc_training(encoder=enc, decoder=dec, model_dir=model_dir)
    gc.collect()
...
def train(
    ...
    model_dir=None,
    ckpt_path="/work/pi_mzink_umass_edu/SPRITE/outputs/ldcast/autoenc/epoch=841-val_rec_loss=0.3662.ckpt",
):
    print("Loading data...")
    ...
    # )
    datamodule = setup_data_casa(for_ae=True)
