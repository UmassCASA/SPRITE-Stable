    ...
    while (data := input_queue.get()) is not None:
        ...
        for member in range(ensemble_size):
            ...
            with contextlib.redirect_stdout(None):
                (s, intermediates) = sampler.sample(num_diffusion_iters, batch_size, gen_shape, x, progbar=False)
...
def create_evaluation_ensemble(
    ...
    data_iter = eval_data.get_data_iter(dataset_id, resolution=resolution, batch_size=batch_size)
    ...
    compute_procs = mp.spawn(
        ldm_process,
        args=(compute_queue, io_queue, ensemble_size, weights_fn, num_diffusion_iters),
        nprocs=num_gpus,
        join=False,
    )

    io_procs = mp.spawn(io_process, args=(io_queue, fn_template, out_dir), nprocs=num_io_procs, join=False)
