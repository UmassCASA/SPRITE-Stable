...
from ldcast.analysis import fss, rank, confmatrix
...
def plot_precip_image(
    ax, R, Rmin=-3.1212, Rmax=4.168956, threshold_mmh=0.1, transform_R=False, grid_spacing=64, std=0.528, mean=-0.051
):
...
def plot_autoencoder_reconstruction(R, R_hat, samples=8, timesteps=4, out_file=None):
    fig = plt.figure(figsize=(2 * timesteps * 2 + 0.5, samples * 2), dpi=150)

    gs = gridspec.GridSpec(
        samples, 2 * timesteps + 1, width_ratios=(1,) * (2 * timesteps) + (0.2,), wspace=0.02, hspace=0.02
    )
...
scale_linestyles = {"1x1": "-", "8x8": "--", "64x64": ":"}
...
def plot_crps(
    ...
    for model, label in zip(models, model_labels):
        for scale in scales:
            ...
            t = np.arange(interval_mins, (len(score) + 0.1) * interval_mins, interval_mins)
            max_t = max(max_t, t[-1])
            ax.plot(t, score, color=color, linestyle=linestyle, label=label)
    ...
    if add_ylabel:
        plt.ylabel("LogCRPS" if log else "CRPS [mm h$^\\mathrm{-1}$]", fontsize=12)
...
def plot_rank_distribution(
    ...
    for scale in scales:
        ...
        for model, label in zip(models, model_labels):
            ...
            ax.plot(x, h, color=color, linestyle=linestyle, label=label_with_score)
        h_ideal = 1 / (num_ensemble_members + 1)
        ax.plot([0, 1], [h_ideal, h_ideal], color=(0.4, 0.4, 0.4), linewidth=1.0)
...
def plot_rank_metric(
    ...
    for model, label in zip(models, model_labels):
        for scale in scales:
            ...
            t = np.arange(interval_mins, (len(score) + 0.1) * interval_mins, interval_mins)
            max_t = max(max_t, t[-1])
            ax.plot(t, score, color=color, linestyle=linestyle, label=label_with_scale)
...
def load_fss(model, scale, use_timesteps, crop_box):
    ...
    with netCDF4.Dataset(fn, "r") as ds:
        ...
        return fss.fractions_skill_score(obs_frac, fc_frac, use_timesteps=use_timesteps)
...
def plot_fss(
    ...
    with concurrent.futures.ProcessPoolExecutor(N_threads) as executor:
        for model in models:
            fss_scale[model] = {}
            for scale in scales:
                fss_scale[model][scale] = executor.submit(load_fss, model, scale, use_timesteps, crop_box)
    ...
    for model, label in zip(models, model_labels):
        ...
        ax.plot(scales, fss_for_model, color=color, label=label)
    ...
    for model in models:
        ...
        with netCDF4.Dataset(fn, "r") as ds:
            for scale in scales:
                ...
                conf_matrix = confmatrix.confusion_matrix_thresholds(fc_frac, obs_frac, prob_thresholds)
    ...
    for model in models:
        ...
        with netCDF4.Dataset(fn, "r") as ds:
            for scale in scales:
                ...
                obs_var = f"obs_frac_scale{scale}"
                obs_frac = np.array(ds[obs_var], copy=False)
                conf_matrix = confmatrix.confusion_matrix_thresholds_leadtime(fc_frac, obs_frac, prob_thresholds)
...
def plot_cost_loss_value(
    ...
    for model in models:
        ...
        with netCDF4.Dataset(fn, "r") as ds:
            for scale in scales:
                ...
                value_scale = []
                for c in cost:
                    v = confmatrix.cost_loss_value(conf_matrix, c, loss, p_clim)
    ...
    for model, label in zip(models, model_labels):
        for scale in scales:
            ...
            ax.plot(cost, score, color=color, linestyle=linestyle, label=label)
