...
class EqualFrequencySampler:
    def __init__(
        ...
        sample_shape,
        time_range_valid,
        time_range_sampling=None,
        timestep_secs=5 * 60,
        random_seed=None,
        preselected_samples=None,
    ):
        binned_patches = bin_classify_patches_parallel(
            bins, *unpack_patches(patch_data), zero_value=patch_data.get("zero_value", 0), scale=patch_data.get("scale")
        )
        complete_ind = indices_with_complete_sample(patch_index, sample_shape, time_range_valid, timestep_secs)
        if time_range_sampling is None:
            time_range_sampling = time_range_valid
        self.starting_ind = [
            starting_indices_for_centers(p, complete_ind, sample_shape, time_range_sampling, timestep_secs)
...
def bin_classify_patches_parallel(
    ...
    for p in range(num_procs):
        ...
        task = dask.delayed(bin_classify_patches)(
            bins,
            patches[pk0:pk1, ...],
            patch_coords[pk0:pk1, ...],
            patch_times[pk0:pk1],
            zero_patch_coords[zk0:zk1, ...],
            zero_patch_times[zk0:zk1],
    ...
    binned_patches = [np.concatenate([cb[i] for cb in chunked_bins], axis=0) for i in range(n_bins)]
    return binned_patches


def indices_with_complete_sample(patch_index, sample_shape, time_range, timestep_secs):
    ...
    complete_ind = Dict.empty(key_type=types.UniTuple(types.int64, 3), value_type=types.uint8)
...
def starting_indices_for_centers(centers, complete_ind, sample_shape, time_range, timestep_secs):
    ...
    @dask.delayed
    def chunk(i):
        starting_ind = Dict.empty(key_type=types.UniTuple(types.int64, 3), value_type=types.uint8)
    ...
    starting_ind = np.concatenate([np.array(list(st_ind.keys())) for st_ind in starting_ind if st_ind], axis=0)
    return starting_ind
