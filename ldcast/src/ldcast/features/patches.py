...
def patch_locations(time_range, patch_box, patch_shape=(32, 32), interval=timedelta(minutes=5), epoch=(1970, 1, 1)):
    patches = {}
    t = time_range[0]
...
def save_patches_radar(patches, archive_path, out_dir, variables=("RZC", "CPCH"), suffix="2020", **kwargs):
    from ..datasets import mchradar

    source_vars = {}
    mchradar_reader = mchradar.MCHRadarReader(archive_path=archive_path, variables=variables, phys_values=False)

    ezc_nonzero_count_func = lambda x: np.count_nonzero((x >= 1) & (x < 251))
    nonzero_count_func = {"RZC": lambda x: np.count_nonzero(x > 1), "CPCH": lambda x: np.count_nonzero(x > 1)}
    zero_value = dict.fromkeys(variables, 0)
    zero_value["RZC"] = 1
    zero_value["CPCH"] = 1
    ...
    save_patches_all(
        ...
        min_nonzeros_to_include=5,
        **kwargs,
    )


def save_patches_dwdradar(
    patches, archive_path, out_dir, variables=("RV",), suffix="2022", patch_shape=(32, 32), **kwargs
):
    from ..datasets import dwdradar

    source_vars = {}
    dwdradar_reader = dwdradar.DWDRadarReader(archive_path=archive_path, variables=variables)
...
def save_patches_ifs(
    ...
    ifs_patches = {dt: pset for (dt, pset) in patches.items() if (dt.minute == dt.second == dt.microsecond == 0)}
    ...
    nonzero_count_func = {v: nonzero_count_func[v.rsplit("-", 1)[0]] for v in variables_with_lag}
    postproc = {f"cin-{lag}": lambda x: np.nan_to_num(x, nan=0.0, copy=False) for lag in lags}
    zero_value = dict.fromkeys(variables_with_lag, 0)
    avg_pool = lambda x: average_pool(x, factor=8, missing=np.nan)
    pool = dict.fromkeys(variables_with_lag, avg_pool)
...
def save_patches_cosmo(patches, archive_path, out_dir, suffix="2020"):
    ...
    cosmonwp_reader = cosmonwp.COSMOCCS4Reader(archive_path=archive_path, cache_size=6000)
    ...
    variables = ["CAPE_MU", "CIN_MU", "SLI", "HZEROCL", "LCL_ML", "MCONV", "OMEGA", "T_2M", "T_SO", "SOILTYP"]
    count_positive = lambda x: np.count_nonzero(x > 0)
    all_nonzero = lambda x: np.prod(x.shape)
    nonzero_count_func = {
    ...
    # Have not been added here, but lets see if it works.
    avg_pool = lambda x: average_pool(x, factor=8, missing=np.nan)
    pool = dict.fromkeys(variables, avg_pool)

    save_patches_all(
        cosmonwp_reader, cosmo_patches, variables, nonzero_count_func, zero_value, out_dir, suffix, pool=pool
    )
...
def save_patches_all(
    ...
    def save_var(var_name):
        ...
        save_patches(
            ...
            zero_patch_times,
            out_fn,
            zero_value=zero_value[var_name],
            scale=var_scale,
        )
...
def get_patches(
    ...
    return (patch_data, patch_coords, patch_times, zero_patch_coords, zero_patch_times)


def save_patches(
    patch_data, patch_coords, patch_times, zero_patch_coords, zero_patch_times, out_fn, zero_value=0, scale=None
):
    with netCDF4.Dataset(out_fn, "w") as ds:
        ...
        var_patch = ds.createVariable(
            "patches", patch_data.dtype, ("dim_patch", "dim_height", "dim_width"), chunksizes=chunksizes, **var_args
        )
        var_patch[:] = patch_data

        var_patch_coord = ds.createVariable("patch_coords", patch_coords.dtype, ("dim_patch", "dim_coord"), **var_args)
        var_patch_coord[:] = patch_coords

        var_patch_time = ds.createVariable("patch_times", patch_times.dtype, ("dim_patch",), **var_args)
        var_patch_time[:] = patch_times

        var_zero_patch_coord = ds.createVariable(
            "zero_patch_coords", zero_patch_coords.dtype, ("dim_zero_patch", "dim_coord"), **var_args
        )
        var_zero_patch_coord[:] = zero_patch_coords

        var_zero_patch_time = ds.createVariable(
            "zero_patch_times", zero_patch_times.dtype, ("dim_zero_patch",), **var_args
        )
...
def load_all_patches(patch_dir, var):
    ...
    patch_data = {}
    keys = ["patches", "patch_coords", "patch_times", "zero_patch_coords", "zero_patch_times"]
    for k in keys:
        patch_data[k] = np.concatenate([fd[k] for fd in file_data], axis=0)
