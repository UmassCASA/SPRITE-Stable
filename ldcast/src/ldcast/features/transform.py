...
def scale_log_norm(scale, threshold=None, missing_value=None, fill_value=0, mean=0.0, std=1.0, dtype=np.float32):
...
class Antialiasing:
    ...
    def __call__(self, img):
        ...
        if img_shape not in self.edge_factors:
            s = convolve(np.ones(img_shape, dtype=np.float32), self.kernel, mode="constant")
            s = 1.0 / s
        ...
        def _convolve_frame(i, j):
            convolve(img[i, j, :, :], self.kernel, mode="constant", output=img_smooth[i, j, :, :])
            img_smooth[i, j, :, :] *= s
...
def default_rainrate_transform(scale):
    scaling = scale_log_norm(scale, threshold=0.1, fill_value=0.02, mean=-0.051, std=0.528, dtype=np.float32)
...
def scale_norm(scale, threshold=None, missing_value=None, fill_value=0, mean=0.0, std=1.0, dtype=np.float32):
...
def one_hot(values):
    ...
    def transform(raw):
        ...
        if (onehot is None) or (onehot.shape[:-1] != raw.shape):
            onehot = np.empty(raw.shape + (num_categories,), dtype=np.float32)
            onehot = np.empty(raw.shape + (num_categories,), dtype=np.uint8)
...
@njit(parallel=True)
def normalize_threshold_array(in_arr, out_arr, mean, std, threshold, fill_value, log=False):
    mean = np.float32(mean)
    inv_std = np.float32(1.0 / std)
