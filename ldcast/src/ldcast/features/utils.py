...
def log_scale_with_zero(range_value, n=65536, dtype=np.float32):
    scale = np.linspace(np.log10(range_value[0]), np.log10(range_value[1]), n - 1)
    scale = np.hstack((0, 10**scale)).astype(dtype)
    return scale


def log_quantize_with_zero(x, range_value, n=65536, dtype=np.uint16):
    scale = log_scale_with_zero(range_value, n=n, dtype=x.dtype)
