...
def convert_var_for_saving(x, fill_value=0.02, min_value=0.05, max_value=118.428, mean=-0.051, std=0.528):
    y = x * std + mean
...
def decode_saved_var_to_rainrate(
    ...
    log_min = np.log10(min_value)
    log_max = np.log10(max_value)
    yc = log_min + (x[mask].astype(np.float32) - 1) * ((log_max - log_min) / 65533)
...
def save_batch(x, y, y_pred, batch_index, fn_template, out_dir, out_fn=None):
    ...
    with netCDF4.Dataset(out_fn, "w") as ds:
        ...
        var_fc = ds.createVariable(
            "forecasts",
            y_pred.dtype,
            ("dim_sample", "dim_channel", "dim_time_future", "dim_h", "dim_w", "dim_member"),
            **var_params,
        )
        var_fc[:] = y_pred

        var_obs_past = ds.createVariable(
            "past_observations", x.dtype, ("dim_sample", "dim_channel", "dim_time_past", "dim_h", "dim_w"), **var_params
        )
        var_obs_past[:] = x
...
def load_batch(fn, decode=True, preproc_fc=None, **kwargs):
    ...
    if decode:
        ...
        y_pred = decode_saved_var_to_rainrate(y_pred, preproc=preproc_fc, **kwargs)

    return (x, y, y_pred)


def load_all_observations(ensemble_dir, decode=True, preproc_fc=None, timeframe="future", **kwargs):
