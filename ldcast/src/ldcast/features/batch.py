...
class BatchGenerator:
    def __init__(
        ...
        self.sources = set.union(*(set(variables[v]["sources"]) for v in self.used_variables))
        ...
        for raw_name_base in self.sources:
            if raw_name_base in forecast_raw_vars:
                raw_names = (rn for rn in raw if rn.startswith(raw_name_base + "-"))
            else:
        ...
        for raw_name in self.forecast_raw_vars:
            patch_index_var = {k: v for (k, v) in self.patch_index.items() if k.startswith(raw_name + "-")}
            self.patch_index[raw_name] = ForecastPatchIndexWrapper(patch_index_var)
        ...
        if (sampler_file is None) or not os.path.isfile(sampler_file):
            ...
            t0 = t1 = None
            for _var_name, var_data in variables.items():
                timesteps = var_data["timesteps"][[0, -1]].copy()
                timesteps[0] -= 1
                ts_secs = timesteps * var_data.get("timestep_secs", self.interval_secs)
            ...
            self.sampler = EqualFrequencySampler(
                sampling_bins,
                raw[primary_raw_var],
                self.patch_index[primary_raw_var],
                sample_shape,
                time_range_valid,
                time_range_sampling=time_range_sampling,
    ...
    def batch(self, samples=None, batch_size=None):
        ...
        for var_name in self.used_variables:
            ...
            raw_data = (self.patch_index[raw_name](t, i0, j0) for raw_name in var_data["sources"])
...
class DeterministicBatchDataset(Dataset):
    def __init__(self, batch_gen, batches_per_epoch, random_seed=None):
        ...
        self.samples = [self.batch_gen.sampler(self.batch_gen.batch_size) for i in range(self.batches_per_epoch)]
...
class PatchIndex:
    ...
    def __init__(
        ...
        self.sample_shape = (self.patch_data.shape[1] * box_size[0], self.patch_data.shape[2] * box_size[1])

        self.patch_index = Dict.empty(key_type=types.UniTuple(types.int64, 3), value_type=types.int64)
        init_patch_index(self.patch_index, patch_coords, patch_times)
        init_patch_index_zero(self.patch_index, zero_patch_coords, zero_patch_times, PatchIndex.IDX_ZERO)

        self._batch = None

    def _alloc_batch(self, batch_size, num_timesteps):
        needs_rebuild = (
            (self._batch is None) or (self._batch.shape[0] < batch_size) or (self._batch.shape[1] < num_timesteps)
        )
        if needs_rebuild:
            del self._batch
            self._batch = np.zeros((batch_size, num_timesteps) + self.sample_shape, self.patch_data.dtype)
        return self._batch
...
@njit
def init_patch_index_zero(patch_index, zero_patch_coords, zero_patch_times, idx_zero):
...
@njit(parallel=True)
def build_batch(
    batch, patch_data, patch_index, t_all, i0_all, i1_all, j0_all, j1_all, bi_size, bj_size, zero_value, missing_value
):
...
class ForecastPatchIndexWrapper(PatchIndex):
    def __init__(self, patch_index):
        ...
        if len(raw_names) != 1:
            raise ValueError("Can only wrap variables with the same base name")
        ...
        if 24 % forecast_interval_hour:
            raise ValueError("24 hours must be a multiple of the forecast interval")
    ...
    def __call__(self, t, i0, j0):
        ...
        lags_hour = (time_from_fc // self.forecast_interval) * self.forecast_interval_hour
