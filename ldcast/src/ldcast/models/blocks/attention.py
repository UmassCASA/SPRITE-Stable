...
class TemporalTransformer(nn.Module):
    def __init__(self, channels, mlp_dim_mul=1, **kwargs):
        super().__init__()
...
class MLP(nn.Sequential):
    def __init__(self, dim, dim_mul=4):
        ...
        sequence = [nn.Linear(dim, inner_dim), nn.SiLU(), nn.Linear(inner_dim, dim)]
        super().__init__(*sequence)


def positional_encoding(position, dims, add_dims=()):
    div_term = torch.exp(torch.arange(0, dims, 2, device=position.device) * (-math.log(10000.0) / dims))
    if position.ndim == 1:
        arg = position[:, None] * div_term[None, :]
    else:
        arg = position[:, :, None] * div_term[None, None, :]

    pos_enc = torch.concat([torch.sin(arg), torch.cos(arg)], dim=-1)
    if add_dims:
        for dim in add_dims:
