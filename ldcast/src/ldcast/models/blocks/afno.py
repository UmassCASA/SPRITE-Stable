...
class AFNO3D(nn.Module):
    ...
    def forward(self, x):
        ...
        o1_real = torch.zeros(
            [B, D, H, W // 2 + 1, self.num_blocks, self.block_size * self.hidden_size_factor], device=x.device
        )
        o1_imag = torch.zeros(
            [B, D, H, W // 2 + 1, self.num_blocks, self.block_size * self.hidden_size_factor], device=x.device
        )
...
class AFNOCrossAttentionBlock3d(nn.Module):
    ...
    def __init__(
        ...
        self.filter = AFNO3D(dim + context_dim, num_blocks, sparsity_threshold, hard_thresholding_fraction)
        self.mlp = Mlp(
