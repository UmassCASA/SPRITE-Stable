...
from ..blocks.afno import AFNOBlock3d, PatchEmbed3d, PatchExpand3d
...
class AFNONowcastNetBasic(nn.Sequential):
    def __init__(self, embed_dim=256, depth=12, patch_size=(4, 4, 4)):
        patch_embed = PatchEmbed3d(embed_dim=embed_dim, patch_size=patch_size)
        # TODO: AFNOBlock was before, but it is not defined in the afno.py file
        blocks = nn.Sequential(*(AFNOBlock3d(embed_dim) for _ in range(depth)))
        patch_expand = PatchExpand3d(embed_dim=embed_dim, patch_size=patch_size)
            ...
            if size_ratio == 1:
                ...
                while size_ratio > 1:
                    scale.append(
                        nn.ConvTranspose3d(
                            dim[i],
                            dim_out if size_ratio == 2 else dim[i],
                            kernel_size=(1, 3, 3),
                            stride=(1, 2, 2),
                            padding=(0, 1, 1),
...
class AFNONowcastNetBase(nn.Module):
    def __init__(
        ...
        for i in range(num_inputs):
            ...
            analysis = nn.Sequential(*(AFNOBlock3d(embed_dim[i]) for _ in range(analysis_depth[i])))
            self.analysis.append(analysis)

        # temporal transformer
        self.use_temporal_transformer = any((ipp != output_patches) for ipp in input_patches)
        if self.use_temporal_transformer:
            self.temporal_transformer = nn.ModuleList(TemporalTransformer(embed_dim[i]) for i in range(num_inputs))

        # data fusion
        self.fusion = FusionBlock3d(embed_dim, input_size_ratios, afno_fusion=afno_fusion, dim_out=embed_dim_out)

        # forecast
        self.forecast = nn.Sequential(*(AFNOBlock3d(embed_dim_out) for _ in range(forecast_depth)))
        ...
        def process_input(i):
            ...
            if self.use_temporal_transformer:
                ...
                pos_enc_output = positional_encoding(
                    torch.arange(1, self.output_patches + 1, device=z.device), self.embed_dim[i], add_dims=(0, 2, 3)
                )
...
class AFNONowcastNet(AFNONowcastNetBase):
    def __init__(self, autoencoder, output_autoencoder=None, **kwargs):
        ...
        self.output_autoencoder = output_autoencoder.requires_grad_(self.train_autoenc)
        self.out_proj = nn.Conv3d(self.embed_dim_out, output_autoencoder.hidden_width, kernel_size=1)
