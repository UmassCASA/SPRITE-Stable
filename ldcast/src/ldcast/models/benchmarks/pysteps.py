...
# # from pysteps import
# # from pysteps import transformation
#
#
# class PySTEPSModel:
#     def __init__(
...
#         out_shape = (self.future_timesteps,) + R.shape[1:] + (self.ensemble_size,)
...
#         (R, _) = transformation.dB_transform(R, threshold=0.1, zerovalue=zerovalue)
...
#         R_f = transformation.dB_transform(R_f, threshold=threshold, inverse=True)[0]
#
...
#         y = [pred(x[i, :, :, :, 0]) for i in range(x.shape[0])]
#         if parallel:
