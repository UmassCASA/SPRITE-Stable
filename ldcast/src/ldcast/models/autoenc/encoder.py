    ...
    def __init__(self, in_dim=1, levels=2, min_ch=64):
        ...
        for i in range(levels):
            ...
            res_block = ResBlock3D(
                in_channels, out_channels, kernel_size=res_kernel_size, norm_kwargs={"num_groups": 1}
            )
            sequence.append(res_block)
            downsample = nn.Conv3d(out_channels, out_channels, kernel_size=(2, 2, 2), stride=(2, 2, 2))
...
class SimpleConvDecoder(nn.Sequential):
    def __init__(self, in_dim=1, levels=2, min_ch=64):
        ...
        for i in reversed(list(range(levels))):
            ...
            upsample = nn.ConvTranspose3d(in_channels, in_channels, kernel_size=(2, 2, 2), stride=(2, 2, 2))
