...
from ldcast.features.io import load_batch
...
def fractions_ensemble(observation, forecasts, threshold, max_scale=256):
    ...
    while True:
        ...
        if scale > max_scale:
            break
        obs = 0.25 * (obs[..., ::2, ::2] + obs[..., 1::2, ::2] + obs[..., ::2, 1::2] + obs[..., 1::2, 1::2])
        fc = 0.25 * (fc[..., ::2, ::2] + fc[..., 1::2, ::2] + fc[..., ::2, 1::2] + fc[..., 1::2, 1::2])
...
def fractions_skill_score(obs_frac, fc_frac, frac_axes=None, fss_axes=None, use_timesteps=None):
    if isinstance(obs_frac, dict):
        return {
            s: fractions_skill_score(
                obs_frac[s], fc_frac[s], frac_axes=frac_axes, fss_axes=fss_axes, use_timesteps=use_timesteps
            )
    ...
    fbs_ref = (obs_frac**2).mean(axis=frac_axes) + (fc_frac**2).mean(axis=frac_axes)
    fss = 1 - fbs / fbs_ref
