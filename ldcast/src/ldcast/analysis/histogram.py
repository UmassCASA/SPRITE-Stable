...
def save_histogram_for_dataset(data_dir, result_fn, bins=(0.05, 120, 100)):
    ...
    with netCDF4.Dataset(result_fn, "w") as ds:
        ...
        obs_var = ds.createVariable("obs_hist", np.uint64, ("dim_bin", "dim_time_future"), **var_params)
        obs_var[:] = obs_hist

        fc_var = ds.createVariable("fc_hist", np.uint64, ("dim_bin", "dim_time_future"), **var_params)
        fc_var[:] = fc_hist

        ds.createDimension("dim_bin_edge", len(bins))
        bin_var = ds.createVariable("bins", np.float64, ("dim_bin_edge",), **var_params)
        bin_var[:] = bins
...
def probability_match_timesteps(obs_hist, fc_hist, bins):
    ...
    return [ProbabilityMatch(obs_hist[:, t], fc_hist[:, t], bins) for t in range(num_timesteps)]
