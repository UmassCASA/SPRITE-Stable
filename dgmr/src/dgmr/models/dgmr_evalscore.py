...
class DGMR(L.LightningModule, NowcastingModelHubMixin):
        ...
        **kwargs,
        ...
        Args:
            ...
            generation_steps: Number of generation steps to use in forward pass, in paper is 6 and the best
                is chosen for the loss. This results in huge amounts of GPU memory though, so less might work better
                for training.
    ...
    def training_step(self, batch, batch_idx):
        ...
        metrics = {
            "train_eval_score": eval_score.detach().cpu(),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Not save the computational graph
        with torch.no_grad():  # OOM Addition
            # Two discriminator steps per generator step
            for _ in range(2):
                predictions = self(images)  # OOM Addition
            ...
            # Optimize Generator #
            ######################
            predictions = [self(images) for _ in range(self.generation_steps)]  # OOM Addition
            # Not needed because during validation there is not backpropogation
            # predictions = [checkpoint(self.forward, images) for _ in range(self.generation_steps)]
        ...
        metrics = {
            "val_eval_score": eval_score.detach().cpu(),
