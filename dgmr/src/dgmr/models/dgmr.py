...
from dgmr.models.common import ContextConditioningStack, LatentConditioningStack
from dgmr.models.discriminators import Discriminator
from dgmr.models.generators import Generator, Sampler
from dgmr.models.hub import NowcastingModelHubMixin
...
from dgmr.models.loss_factory import RegularizerLossFactory
from dgmr.models.weight_ceiling import weight_fn
from dgmr.models.utils import record_metrics_to_csv


class DGMR(L.LightningModule, NowcastingModelHubMixin):
    ...
    def __init__(
        ...
        generation_steps: int = 6,  # Reduce if OOM
        regularizer_type: str = "grid_cell",
        regularizer_lambda: float = 20.0,
        ...
        **kwargs,
        ...
        Args:
            ...
            regularizer_lambda: User-defined lambda for the regularization loss
            regularizer_type: Type of regularization loss to use, see dgmr/loss_factory.py for options
            ...
            generation_steps: Number of generation steps to use in forward pass, in paper is 6 and
                the best is chosen for the loss this results in huge amounts of GPU memory though, so
                less might work better for training.
            ...
            context_channels: Number of channels that the context space should be reshaped to,
                input dimension into ConvGRU, also affects the number of channels for other linked inputs/outputs
            metrics_path: Path to save the metrics
            job_id: Job ID for the metrics
        """
        super().__init__()
        config = locals()
        ...
        regularizer_type = self.config["regularizer_type"]
        regularizer_lambda = self.config["regularizer_lambda"]
        ...
        self.regularizer = RegularizerLossFactory.create_regularizer(
            regularizer_type=regularizer_type, weight_fn=weight_fn, precip_weight_cap=precip_weight_cap
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        predictions_mean = torch.stack(predictions, dim=0).mean(dim=0)
        regularizer_loss = self.regularizer(predictions_mean, future_images)
        ...
        # Combine all losses
        generator_loss = generator_disc_loss + self.regularizer_lambda * regularizer_loss
        ...
        metrics = {
            ...
            "train_regularizer_loss": regularizer_loss.detach().cpu(),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():  # Not save the computational graph
            # Two discriminator steps per generator step
            for _ in range(2):
                predictions = self(images)
                # Not needed because during validation there is not backpropogation
                # predictions = checkpoint(self.forward, images)
        ...
        metrics = {
            ...
            "val_regularizer_loss": regularizer_loss.detach().cpu(),
