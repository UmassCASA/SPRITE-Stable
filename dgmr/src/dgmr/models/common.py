...
from dgmr.models.layers import AttentionLayer
from dgmr.models.layers.utils import get_conv_layer
...
class GBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.last_conv_3x3 = spectral_norm(
            conv2d(in_channels=input_channels, out_channels=output_channels, kernel_size=3, padding=1),
            eps=spectral_normalized_eps,
        )
...
class DBlock(torch.nn.Module):
    def __init__(
        ...
        Args:
            ...
            keep_same_output: Whether the output should have the same spatial dimensions as input,
                if False, downscales by 2
        """
        super().__init__()
...
class LatentConditioningStack(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self, shape: (int, int, int) = (8, 8, 8), output_channels: int = 768, use_attention: bool = True, **kwargs
    ):
        """
        ...
        self.conv_3x3 = spectral_norm(
            torch.nn.Conv2d(in_channels=shape[0], out_channels=shape[0], kernel_size=(3, 3), padding=1)
        )
        ...
        if self.use_attention:
            self.att_block = AttentionLayer(input_channels=output_channels // 4, output_channels=output_channels // 4)
