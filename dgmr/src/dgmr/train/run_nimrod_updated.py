...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Training Script for DGMR")
    parser.add_argument("--nodes", type=int, required=True, help="Number of nodes for distributed training")
    parser.add_argument("--gpus", type=int, required=True, help="Number of GPUs per node")
    parser.add_argument("--partition", type=str, required=True, help="Partition name for output directory")
    parser.add_argument("--job_id", type=int, required=True, help="Job ID for logging")
    parser.add_argument("--dirpath", type=str, required=True, help="Directory path for output")
...
class DGMRDataModule(LightningDataModule):
    ...
    def val_dataloader(self):
        dataloader = DataLoader(TFDataset(split="validation"), batch_size=self.batch_size, num_workers=self.num_workers)
        return dataloader


def start_training(nodes, gpus, job_id, dirpath, modelname="DGMR-Nimrod"):
    ...
    checkpoint_callback_nth_epochs = ModelCheckpoint(
        every_n_epochs=50,
        dirpath=dirpath,
        filename=modelname + "-{epoch:03d}",
        save_top_k=-1,  # This ensures that all checkpoints are saved and not just the top-k
        verbose=True,
    )

    if nodes > 1 or gpus > 1:
        selected_strategy = "ddp_find_unused_parameters_true"
    else:
        selected_strategy = None

    trainer = Trainer(
        # max_time={"days": 7},  # Train for a week
        max_time={"days": 0, "hours": 11},
        logger=wandb_logger,
        callbacks=[model_checkpoint, checkpoint_callback_nth_epochs],
        num_nodes=nodes,
        devices=gpus,
        accelerator="gpu",
        precision=32,
    ...
    model = DGMR(generation_steps=6, metrics_path=os.path.join(dirpath, "metrics"), job_id=job_id)
    datamodule = DGMRDataModule(batch_size=16, num_workers=0)
    trainer.fit(model, datamodule)


if __name__ == "__main__":
    args = parse_arguments()
    modelname = f"DGMR-Nirmod-{args.job_id}"
    start_training(args.nodes, args.gpus, args.job_id, args.dirpath, modelname)
