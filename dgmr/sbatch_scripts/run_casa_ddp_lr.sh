...
partition="gpu"       # SLURM partition (e.g., gpu, uri-gpu, gpu-preempt)
project="LearningRate_big" # Project name for outputs and logging
...
for combo in "${!lr_combinations[@]}"; do
    IFS=" " read -r gen_lr disc_lr <<< "${lr_combinations[$combo]}"
