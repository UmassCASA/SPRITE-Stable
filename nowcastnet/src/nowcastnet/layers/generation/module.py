...
class LBlock(nn.Module):
    """
    Based on L Block shown in Extended Data Fig. 1 in NowcastNet paper:
      1) upper branch: BN -> 3x3 Conv
      2) lower branch: BN -> ReLU -> 3x3 Conv
      3) main branch = upper branch + lower branch
    """

    def __init__(self, in_channel, out_channel):
        super().__init__()
        # upper branch
        self.bn_top = nn.BatchNorm2d(num_features=in_channel)
        self.conv_top = nn.Conv2d(in_channel, out_channel, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

        # lower branch
        self.bn_bottom = nn.BatchNorm2d(num_features=in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.conv_bottom = nn.Conv2d(in_channel, out_channel, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

    def forward(self, x):
        # upper branch: BN -> Conv
        out_top = self.bn_top(x)
        out_top = self.conv_top(out_top)

        # lower branch: BN -> ReLU -> Conv
        out_bottom = self.bn_bottom(x)
        out_bottom = self.relu(out_bottom)
        out_bottom = self.conv_bottom(out_bottom)
