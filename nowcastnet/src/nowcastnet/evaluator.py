...
def test_pytorch_loader(model, test_input_handle, configs, itr):
    ...
    for batch_id, test_ims in enumerate(test_input_handle):
        ...
        def save_plots(
            field, labels, res_path, figsize=None, vmin=0, vmax=10, cmap="viridis", npy=False, **imshow_args
        ):
            for i, data in enumerate(field):
                ...
                plt.savefig(f"{res_path}/{labels[i]}.png")
                plt.close()
                if npy:
                    with open(f"{res_path}/{labels[i]}.npy", "wb") as f:
                        np.save(f, data[..., 0])
        ...
        if batch_id <= configs.num_save_samples:
            ...
            save_plots(
                img_gen_plot,
                labels=[f"pd{i + 1}" for i in range(9, configs.total_length)],
                res_path=path,
                vmin=vis_info["vmin"],
                vmax=vis_info["vmax"],
            )
